...
app_settings = _load_settings(_init_db)
cyber_devices = _load_cyber_devices(_init_db)
_init_db.close()
...
def generate_live_telemetry():
    """
    Background daemon thread — runs the local Edge AI processing pipeline.
    ...
    """
    global demo_time, cyber_devices, _main_loop

    while _main_loop is None:
        time.sleep(0.05)
    ...
    while True:
        ...
        try:
            ...
            raw_telemetry = asyncio.run(
                telemetry_driver.fetch_telemetry_tick(machines, demo_time)
            )
            ...
            for m_dict in raw_telemetry:
                ...
                ai_results = analyze_telemetry(m_dict, history, {
                    "warning": app_settings["healthWarnThreshold"],
                    "critical": app_settings["healthCritThreshold"]
                })
...
async def startup_event():
    ...
    t = threading.Thread(target=generate_live_telemetry, daemon=True)
    t.start()
